...
public class LibraryEntryCommandTest extends AbstractProjectsManagerBasedTest {
  ...
  @Test
  public void shouldFindJUnitEntry() throws Exception {
    ...
    JarEntry entry = LibraryEntryCommand.execute(singletonList(params), new NullProgressMonitor());

    assertNotNull(entry);
    assertEquals("junit.extensions.TestSetup", entry.getPath());
    assertEquals("TestSetup.class", entry.getName());
    assertEquals("CLASS_FILE", entry.getEntryType());
  }
